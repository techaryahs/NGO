...
class _DashboardTabState extends State<DashboardTab> {
  ...
  void _subscribeStreams() {
    ...
    _salariesSub = _service.getSalariesStream().listen((data) {
      if (mounted) setState(() { _salaries = data; _loading = false; });
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final monthlyPurchases = _purchases.where((p) => p.purchaseDate.month == now.month && p.purchaseDate.year == now.year).fold<double>(0, (sum, p) => sum + p.totalAmount);
    final monthlyGeneralExpenses = _expenses.where((e) => e.expenseDate.month == now.month && e.expenseDate.year == now.year).fold<double>(0, (sum, e) => sum + e.totalAmount);
    final totalExpenseMonth = monthlyPurchases + monthlyGeneralExpenses;

    // Yearly Expenses
    final yearlyPurchases = _purchases.where((p) => p.purchaseDate.year == now.year).fold<double>(0, (sum, p) => sum + p.totalAmount);
    final yearlyGeneralExpenses = _expenses.where((e) => e.expenseDate.year == now.year).fold<double>(0, (sum, e) => sum + e.totalAmount);
    final totalExpenseYear = yearlyPurchases + yearlyGeneralExpenses;

    // Salaries This Month
    final totalSalariesMonth = _salaries.where((s) => s.createdAt.month == now.month && s.createdAt.year == now.year).fold<double>(0, (sum, s) => sum + s.netSalary);

    // Pending Payments Count (status = 'Pending' or 'Partial' across transactions)
    final pendingPurchases = _purchases.where((p) => p.paymentDetails.paymentStatus != 'Paid').length;
    final pendingExpenses = _expenses.where((e) => e.paymentDetails.paymentStatus != 'Paid').length;
    final pendingSalaries = _salaries.where((s) => s.paymentDetails.paymentStatus != 'Paid').length;
    final totalPendingPayments = pendingPurchases + pendingExpenses + pendingSalaries;
    ...
    for (int i = 5; i >= 0; i--) {
      ...
      final monthPurchases = _purchases.where((p) => p.purchaseDate.month == targetDate.month && p.purchaseDate.year == targetDate.year).fold<double>(0, (sum, p) => sum + p.totalAmount);
      final monthExpenses = _expenses.where((e) => e.expenseDate.month == targetDate.month && e.expenseDate.year == targetDate.year).fold<double>(0, (sum, e) => sum + e.totalAmount);
    ...
    for (final e in _expenses) {
      categoryBreakdown[e.category] = (categoryBreakdown[e.category] ?? 0.0) + e.totalAmount;
    }
